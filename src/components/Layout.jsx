...
function Layout({ session }) {
  ...
  const navLinkStyles = ({ isActive }) => ({
    ...
    textDecoration: 'none',
    // marginRight: '20px', // Removido daqui, será aplicado pelo gap no container
  });

  const mobileNavLinkStyles = ({ isActive }) => ({
    ...navLinkStyles({ isActive }),
    display: 'block',
    padding: '12px 20px', // Ajustei o padding para mobile
    ...
    <div>
      <style>{`
        .desktop-nav-links {
          display: flex;
          align-items: center;
          gap: 20px; /* Adicionado gap para espaçamento entre links desktop */
        }
        .mobile-menu-button {
          display: none;
          background: none;
        ...
        @media (max-width: 768px) {
          .desktop-nav-links > a, 
          .desktop-nav-links > span { /* Esconde links e saudação no desktop */
             display: none;
          }
          /* Para garantir que o botão de logout desktop não apareça no mobile */
          .user-info-desktop .logout-button-desktop {
             display: none;
          }
          .mobile-menu-button {
            display: block;
      ...
      <nav style={{
        ...
        <div className="desktop-nav-links"> {/* Removido style={{ gap: '20px' }} daqui, já está no CSS */}
          {session && (
            <>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {!session ? (
            // Oculta o link de Login desktop em telas pequenas usando a classe para aplicar display:none da media query
            <NavLink to="/auth" className="desktop-nav-links" style={navLinkStyles} onClick={closeMobileMenu}>
              Login
            </NavLink>
          ) : (
            // Oculta user-info e botão logout desktop em telas pequenas usando a classe
            <span className="desktop-nav-links user-info-desktop" style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ background: '#61dafb', border: 'none', color: '#20232a', cursor: 'pointer', padding: '8px 15px', borderRadius: '4px', fontWeight: 'bold' }}
              >
                Sair
              </button>
            </span>
      ...
      {session && (
        <div className={`mobile-nav-menu ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <div style={{ padding: '10px 20px', borderTop: '1px solid #444', marginTop: '10px' }}>
            ...
            <button
              onClick={handleLogout}
              style={{ background: '#61dafb', width: '100%', border: 'none', color: '#20232a', cursor: 'pointer', padding: '10px 15px', borderRadius: '4px', fontWeight: 'bold' }}
            >
              Sair
            </button>
          </div>
