// src/features/partnerWithdrawals/components/WithdrawalsListTable.jsx (VERSÃO ATUALIZADA)
...
const WithdrawalsListTable = ({ withdrawals, isLoading, handleEdit, handleDelete, handleSort, sortColumn, sortDirection }) => {
      ...
      <table className={styles.listTable}>
        <thead>
          <tr>
            ...
            {/* MUDANÇA: Ordenando pela coluna correta da VIEW */}
            <th onClick={() => handleSort('partner_name')}>Sócio <span className={styles.sortIcon}>{renderSortIcon('partner_name')}</span></th>
          ...
          {withdrawals.length === 0 && !isLoading ? (
            ...
            withdrawals.map((wd) => (
              <tr key={wd.withdrawal_id}>
                {/* MUDANÇA: Adicionados data-labels para responsividade */}
                <td data-label="Data">{new Date(wd.withdrawal_date + 'T00:00:00Z').toLocaleDateString()}</td>
                {/* MUDANÇA: Usando a coluna correta da VIEW */}
                <td data-label="Sócio">{wd.partner_name || 'N/A'}</td>
