...
// ================================================================
// ========= INÍCIO DAS ALTERAÇÕES FINAIS NA BARRA ================
// ================================================================
const PaymentProgressBar = ({ paid, total }) => {
  ...
  // MUDANÇA 2: Lógica para definir a cor do contorno
  let borderColor = '#28a745'; // Verde (pago) por padrão
  let barColor = '#28a745';
  let textColor = '#fff';

  if (percentage < 99.9) {
    if (paidValue === 0) {
      borderColor = '#dc3545'; // Vermelho para zerado
      barColor = '#e9ecef';
      textColor = '#495057';
    } else {
      borderColor = '#007bff'; // Azul para parcial
      barColor = '#007bff';
      textColor = '#fff';
    }
  }
  ...
  const barFillStyle = {
    ...
    borderRadius: '2px', // Borda interna um pouco menor
    transition: 'width 0.4s ease-in-out',
  };
  
  // MUDANÇA 1: Adicionando sombra no texto para legibilidade
  const textStyle = {
    position: 'absolute',
    width: '100%',
    textAlign: 'center',
    ...
    color: textColor,
    lineHeight: '22px',
    textShadow: '1px 1px 2px rgba(0, 0, 0, 0.6)', // Sombra para contraste
  };

  const saldoStyle = {
    fontSize: '0.75rem',
    color: saldo > 0.005 ? '#dc3545' : '#6c757d',
    fontWeight: 'bold',
    marginTop: '4px',
    textAlign: 'center'
  };
...
// ==============================================================
// ========= FIM DAS ALTERAÇÕES FINAIS NA BARRA =================
// ==============================================================

const SalesListTable = ({ sales, loading, filterText, setFilterText, handleEdit, handleDelete, openPaymentModal }) => {
  // ... (o resto do componente da tabela permanece igual)
  return (
    <div>
      <h2>Vendas Registradas</h2>
      <input
        type="text"
        placeholder="Filtrar vendas por ID, Cliente, CC, etc..."
        value={filterText}
      ...
      {!loading && sales.length === 0 && <p>Nenhuma venda encontrada.</p>}
              ...
              return (
                <tr key={sale.sale_id}>
                  ...
                  <td className={styles.actionsCell} data-label="Ações">
                    <div>
                      <button onClick={() => openPaymentModal(sale)} title="Ver/Adicionar Pagamentos"><PaymentsIcon /></button>
                      <button onClick={() => handleEdit(sale)} title="Editar Venda"><EditIcon /></button>
                      <button onClick={() => handleDelete(sale.sale_id, saleIdentifier)} title="Excluir Venda"><DeleteIcon /></button>
                    </div>
                  </td>
                </tr>
              );
